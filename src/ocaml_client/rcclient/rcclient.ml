...
module type S = sig
  ...
  val put : t -> bytes -> bytes -> (unit, [`Msg of string]) Deferred.Result.t

  val get : t -> bytes -> (unit, [`Msg of string]) Deferred.Result.t
end
...
module Make (Cli : S) : sig
  ...
  let perform op client clientid queue_start i =
    [%log.debug logger "Submitting request" (i : int)] ;
    let client_start = Unix.gettimeofday () in
    let%bind res =
      let keybuf = Bytes.create 8 in
      match op with
      | MT.Put {key; value} ->
          Stdlib.Bytes.set_int64_ne keybuf 0 key ;
          Cli.put client keybuf value
      | MT.Get {key} ->
          Stdlib.Bytes.set_int64_ne keybuf 0 key ;
          Cli.get client keybuf
    in
    let end_ = Unix.gettimeofday () in
    [%log.debug logger "Performed" ~op:(i : int)] ;
    let optype = match op with MT.Put _ -> "Put" | MT.Get _ -> "Get" in
    let err = match res with Ok _ -> "" | Error (`Msg s) -> s in
    return
      MT.
        { response_time= end_ -. client_start
        ; client_start
        ; queue_start
        ; end_
        ; clientid
  ...
  let recv_from_br (br : Reader.t) =
    ...
    let%bind () =
      match%bind.Deferred Reader.really_read br payload_buf ~pos:0 ~len with
      | `Eof _ as v ->
          fail v
      | `Ok ->
          return ()
  ...
  let send payload outfile =
    ...
    Writer.write_bytes outfile payload_buf ~pos:0
      ~len:(Bytes.length payload_buf)
  ...
  let run client id result_pipe =
      ...
      Pipe.unfold ~init:() ~f:(fun () ->
          ...
          | Ok v -> (
            ...
            | Finalise {msg} ->
                ...
                None
            | Start {msg= _msg} ->
                failwith "Got start during preload phase"
            | Op op ->
                Some (op, ()) ))
    in
    let%bind op_list =
      Pipe.fold pipe ~init:[] ~f:(fun ops v ->
          ...
          | MT.{prereq= true; _} ->
              let%bind _res =
                perform v.op_type client id (Unix.gettimeofday ()) (-1)
              in
              return ops
          | _ ->
              v :: ops |> return)
    in
    let op_list =
      List.sort op_list ~compare:(fun a b -> Float.compare a.start b.start)
    in
    ...
    let%bind _got_start =
      ...
      | Ok (Start {msg}) ->
          ...
          Deferred.unit
      | _ ->
          failwith "Got something other than a ready!"
    in
    ...
    let foldi test_start i acc MT.{prereq= _; start= offset; op_type= op} =
      ...
      let res =
        perform op client id
          (start |> Time.to_span_since_epoch |> Time.Span.to_sec)
          i
      in
      return (res :: acc)
    in
    let%bind res =
      Deferred.List.foldi op_list ~init:[] ~f:(foldi @@ Time.now ())
    in
